...
public:
    explicit MainWindow(QWidget* parent = nullptr);  // 构造函数，可指定父窗口
...
private:
    ...
    // ============ 影像显示 ============
    void displayRaster(const std::shared_ptr<RasterLayer>& raster, int bandIndex);  // 在 QGraphicsView 中显示影像
    ...
    std::shared_ptr<RasterLayer> selectedRaster() const;  // 获取当前选中的 RasterLayer（非影像返回 nullptr）
