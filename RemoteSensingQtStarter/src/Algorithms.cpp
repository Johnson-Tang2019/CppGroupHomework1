...
std::vector<AlgorithmParameter> HistogramAlgorithm::parameterSchema() const {
    return {{QStringLiteral("bins"), QStringLiteral("分箱数"), QStringLiteral("256"),
             QStringLiteral("直方图统计区间数量")},
            {QStringLiteral("ignoreNoData"), QStringLiteral("忽略 NoData"), QStringLiteral("true"),
...
std::vector<AlgorithmParameter> HistogramEqualizationAlgorithm::parameterSchema() const {
    return {{QStringLiteral("band"), QStringLiteral("处理波段"), QStringLiteral("selected"),
             QStringLiteral("可处理当前波段或当前 RGB 显示影像")},
            {QStringLiteral("clipLimit"), QStringLiteral("CLAHE 限幅"), QStringLiteral("2.0"),
             QStringLiteral("进阶可实现 CLAHE")}};
}
...
std::vector<AlgorithmParameter> FeatureExtractionAlgorithm::parameterSchema() const {
    return {{QStringLiteral("method"), QStringLiteral("方法"), QStringLiteral("ORB"),
             QStringLiteral("可选 ORB/SIFT/AKAZE")},
            {QStringLiteral("maxFeatures"), QStringLiteral("最大特征数"), QStringLiteral("2000"),
...
std::shared_ptr<DemLayer> DemReconstructionPipeline::reconstruct(const Inputs &inputs) const {
    Q_UNUSED(inputs)
    throw std::runtime_error("TODO: use feature matching, stereo rectification and disparity/depth "
