...
const QHash<QString, QHash<QString, QString>> &catalog() {
    static const QHash<QString, QHash<QString, QString>> table = {
        ...
        {QStringLiteral("action.load_raster"),
         {{QStringLiteral("zh"), QStringLiteral("加载遥感影像（可多选）")},
          {QStringLiteral("en"), QStringLiteral("Load Raster (multi-select)")}}},
        ...
        {QStringLiteral("log.startup"),
         {{QStringLiteral("zh"),
           QStringLiteral("Starter 已启动：当前版本提供多波段影像、参数化算法、DEM/正射流程的工程骨架。")},
          {QStringLiteral("en"),
           QStringLiteral(
               "Starter launched: this build provides multi-band raster workflows, parametric algorithms, "
        ...
        {QStringLiteral("action.index_calc"),
         {{QStringLiteral("zh"), QStringLiteral("计算 NDVI/NDWI/NDBI/MNDWI/NDCI/NDTI/TSS/TLI/FUI/PC/CDOM...")},
          {QStringLiteral("en"), QStringLiteral("Compute NDVI/NDWI/NDBI/MNDWI/NDCI/NDTI/TSS/TLI/FUI/PC/CDOM...")}}},
        ...
        {QStringLiteral("action.swipe_compare"),
         {{QStringLiteral("zh"), QStringLiteral("前后影像滑动对比...")},
          {QStringLiteral("en"), QStringLiteral("Before/After Swipe Compare...")}}},
        ...
        {QStringLiteral("log.panorama_qt_failed_gdal"),
         {{QStringLiteral("zh"), QStringLiteral("Qt 读取 360 街景图失败：%1；尝试使用栅格读取兜底。")},
          {QStringLiteral("en"), QStringLiteral("Qt failed to read 360 panorama: %1; trying raster fallback.")}}},
        {QStringLiteral("log.panorama_gdal_failed"),
         {{QStringLiteral("zh"), QStringLiteral("栅格兜底读取也失败：%1")},
          {QStringLiteral("en"), QStringLiteral("Raster fallback also failed: %1")}}},
